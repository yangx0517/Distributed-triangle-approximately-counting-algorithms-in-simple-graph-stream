...
double run_mpi(const char* filename, MPIIO &hIO, int workerNum, int m_pHash, int memSize, int lenBuf, double tolerance, unsigned int seed, std::vector<float> & oLocalCnt, double &srcCompCost, double &workerCompCostMax, double &workerCompCostSum)
{
    ...
    if (hIO.isMaster())
    {
        ...
        while (parser.getEdge(edge)) // Stream edges
        {
            if(edge.src != edge.dst) {
                ...
                VID m  = rand() % m_pHash;
        		edge.m = m;
                hIO.bCastEdge(edge, dst1, dst2);   //广播 
            }
        }
        ...
        globalCnt = globalCnt * m_pHash * m_pHash /workerNum;
        for(auto it = oLocalCnt.begin(); it != oLocalCnt.end(); ++it)
        {
            *it  = *it * m_pHash * m_pHash /workerNum;
        }
    ...
    }else // Worker part
    {
        ...
        {
        	//cout << "u: " << edge.src << "  v:" << edge.dst << endl;

            worker.processEdge(edge, workerNum, hIO.getWorkerId(), m_pHash);
...
void run_exp (const char* input, const char* outPath, MPIIO &hIO, int workerNum, int m_pHash, int memSize, int repeat, int bufLen, double tolerance)
{
    ...
    for(int i =0 ; i < repeat; i++) {
        ...
        if (hIO.isMaster()) {
			...
			double globalCnt = run_mpi(input, hIO, workerNum,m_pHash, memSize, bufLen, tolerance, seed + repeat * workerNum * i, nodeToCnt, srcCompCost, workerCompCostMax, workerCompCostSum);
            ...
            run_mpi(input, hIO, workerNum, m_pHash, memSize, bufLen, tolerance, seed + repeat * workerNum * i, nodeToCnt, srcCompCost, workerCompCostMax, workerCompCostSum);
        }
    }
}
