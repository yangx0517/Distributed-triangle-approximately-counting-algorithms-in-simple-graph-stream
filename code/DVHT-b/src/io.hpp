...
public:
	...
	bool isMaster(); //判断是否是主进程 

	// whether this thread is a valid worker or not
	// bool isActiveWorker();

	// worker Id
	MID getWorkerId(); //返回工作进程号 
	...
	// Edge
	bool sendEdge(const Edge &iEdge, int dst); //发送边 
	
	bool bCastEdge(Edge& iEdge, MID dst1, MID dst2); //广播边 
	bool recvEdge(Edge &oEdge); //接收边 

	// Receiving data
	bool sendCnt(double gCnt, unordered_map<VID, float> &lCnt); //发送计数结果 
    bool recvCnt(VID maxVId, double &gCnt, std::vector<float> &lCnt); //接收计数结果 

	// CPU Time used for MPI communication
	double getIOCPUTime(); //计算通信时间 

	// send computational time info
	bool sendTime(double compTime); //发送通信时间 
	bool recvTime(double &compTimeMax, double &compTimeSum); //接收通信时间 

	// Control flow
	bool sendEndSignal();

	// Get variable
	int getRank(){ return rank;} //返回本进程号 
	int getSzProc(){ return szProc;} //返回进程数 
