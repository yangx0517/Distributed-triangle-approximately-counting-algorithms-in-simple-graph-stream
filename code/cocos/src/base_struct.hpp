...
struct Edge 
{
	static const unsigned short szAttr 	= 4;
	...
	Edge(): src(INVALID_VID), dst(INVALID_VID),hashSrc(0), hashDst(0) {}
	Edge(VID iSrc, VID iDst): src(iSrc), dst(iDst) {}

	//Edge(const Edge &iEdge): src(iEdge.src), dst(iEdge.dst) {}
	inline bool operator==(const Edge& iEdge) const //ÖØÔØEdgeµÄ == ÔËËã·û 
	{
