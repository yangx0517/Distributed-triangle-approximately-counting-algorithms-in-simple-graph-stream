...
MPIIO::MPIIO(int &argc, char** &argv)//, bit(0), lenCQ(1), cqit(0)
{
	...
	int          lenAttr[Edge::szAttr] = {1, 1, 1, 1};
	MPI_Datatype arrType[Edge::szAttr] = {MPI_UNSIGNED, MPI_UNSIGNED,MPI_UNSIGNED, MPI_UNSIGNED}; //数据类型 

	MPI_Aint     offsets[Edge::szAttr]; //地址偏移类型 
	offsets[0] = offsetof(Edge, src);
	offsets[1] = offsetof(Edge, dst);
	offsets[2] = offsetof(Edge, hashSrc);
	offsets[3] = offsetof(Edge, hashDst);
...
bool MPIIO::sendEdge(const Edge &iEdge, int dst)
{
	Edge tmpEdge(iEdge);
