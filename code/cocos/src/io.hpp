...
public:
	...
	std::unordered_map<VID, VID> load;
	std::unordered_map<VID, VID> hashMap;//ӳ��� 
	...
	// Edge
	bool sendEdge(const Edge &iEdge, int dst); //���ͱ� 
	
	bool bCastEdge(Edge& iEdge, MID dst1, MID dst2); //�㲥�� 
	bool recvEdge(Edge &oEdge); //���ձ� 

	// Receiving data
	bool sendCnt(double gCnt, unordered_map<VID, float> &lCnt); //���ͼ������ 
    bool recvCnt(VID maxVId, double &gCnt, std::vector<float> &lCnt); //���ռ������ 

	// CPU Time used for MPI communication
	double getIOCPUTime(); //����ͨ��ʱ�� 

	// send computational time info
	bool sendTime(double compTime); //����ͨ��ʱ�� 
	bool recvTime(double &compTimeMax, double &compTimeSum); //����ͨ��ʱ�� 

	// Control flow
	bool sendEndSignal();

	// Get variable
	int getRank(){ return rank;} //���ر����̺� 
	int getSzProc(){ return szProc;} //���ؽ����� 
